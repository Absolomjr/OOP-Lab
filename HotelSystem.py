...
class HotelApp:
    ...
    def login_window(self):
        ...
        self.login_win.config(bg="lightblue")

        tk.Label(self.login_win, text="Username:", bg="lightblue").grid(row=0, column=0, pady=10, padx=10, sticky='e')
        self.username_entry = tk.Entry(self.login_win)
        self.username_entry.grid(row=0, column=1, pady=10, padx=10)

        tk.Label(self.login_win, text="Password:", bg="lightblue").grid(row=1, column=0, pady=10, padx=10, sticky='e')
        self.password_entry = tk.Entry(self.login_win, show="*")
        self.password_entry.grid(row=1, column=1, pady=10, padx=10)

        tk.Button(self.login_win, text="Login", command=self.login, bg="green", fg="white").grid(row=2, column=0, columnspan=2, pady=20)
    ...
    def main_app(self):
        ...
        # Guest Details Entry
        tk.Label(self.window, text="Guest Name:", bg="lightgray").grid(row=0, column=0, pady=10, padx=10, sticky='e')
        self.guest_name_entry = tk.Entry(self.window)
        self.guest_name_entry.grid(row=0, column=1, pady=10, padx=10)

        tk.Label(self.window, text="Contact Number:", bg="lightgray").grid(row=1, column=0, pady=10, padx=10, sticky='e')
        self.contact_entry = tk.Entry(self.window)
        self.contact_entry.grid(row=1, column=1, pady=10, padx=10)

        tk.Label(self.window, text="Check-In Date (YYYY-MM-DD):", bg="lightgray").grid(row=2, column=0, pady=10, padx=10, sticky='e')
        self.check_in_date_entry = tk.Entry(self.window)
        self.check_in_date_entry.grid(row=2, column=1, pady=10, padx=10)

        tk.Label(self.window, text="Check-Out Date (YYYY-MM-DD):", bg="lightgray").grid(row=3, column=0, pady=10, padx=10, sticky='e')
        self.check_out_date_entry = tk.Entry(self.window)
        self.check_out_date_entry.grid(row=3, column=1, pady=10, padx=10)

        tk.Label(self.window, text="Select Room Type:", bg="lightgray").grid(row=4, column=0, pady=10, padx=10, sticky='e')
        self.room_type_combo = ttk.Combobox(self.window, values=["Single Room", "Premium Room", "Suite Room"])
        self.room_type_combo.grid(row=4, column=1, pady=10, padx=10)
        self.room_type_combo.set("Single Room")

        tk.Button(self.window, text="Make Reservation", command=self.make_reservation, bg="blue", fg="white").grid(row=5, column=0, columnspan=2, pady=20)

        tk.Button(self.window, text="Show Available Rooms", command=self.show_available_rooms, bg="blue", fg="white").grid(row=6, column=0, columnspan=2, pady=10)
        tk.Button(self.window, text="Check In", command=self.check_in_guest, bg="blue", fg="white").grid(row=7, column=0, columnspan=2, pady=10)
        tk.Button(self.window, text="Check Out", command=self.check_out_guest, bg="blue", fg="white").grid(row=8, column=0, columnspan=2, pady=10)
        tk.Button(self.window, text="Make Payment", command=self.make_payment, bg="blue", fg="white").grid(row=9, column=0, columnspan=2, pady=10)
